...
public class TowerObject : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(info.atkType == 1)
        {
            //没有目标 或者目标死亡 或者目标超出攻击距离
            if (targetObj == null || targetObj.isDead || Vector3.Distance(this.transform.position,
                targetObj.transform.position) > info.atkRange)
            {
                targetObj = GameLevelMgr.Instance.FindMonster(this.transform.position, info.atkRange);

            }
            //如果没有找到任何可以攻击的对象 应该直接返回
            if (targetObj == null)
                return;
            //得到怪物位置 偏移y的目的是希望 炮台头部和角色身体平行
            monsterPos = targetObj.transform.position;
            monsterPos.y = head.position.y;
            //让炮台的头部旋转起来
            head.rotation = Quaternion.Slerp(head.rotation,
                Quaternion.LookRotation(monsterPos - head.position), roundSpeed * Time.deltaTime);
        ...
        else
        {
            ...
            if(targetObjs.Count >0&&Time.time - nowTime >=info.atkRange)
            {
                //创建开火特效
                GameObject effObj = Instantiate(Resources.Load<GameObject>(info.eff)
                    , transform.position+Vector3.up, transform.rotation);
                Destroy(effObj, 0.2f);
